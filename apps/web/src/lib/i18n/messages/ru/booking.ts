import type { Messages } from '../types';

export const booking: Messages['booking'] = {
	myBookings: 'Мои бронирования',
	noBookings: 'Бронирований пока нет',
	noBookingsDescription: 'Посмотрите события и забронируйте следующий тур.',
	browseEvents: 'Смотреть события',
	paymentSuccess: 'Платеж прошел успешно. Ваша бронь скоро будет подтверждена.',
	bookingRef: 'Код брони',
	bookingRefTip: 'Сохраните код — с его помощью вы сможете найти вашу бронь',
	spot: 'место',
	spots: 'места',
	total: 'всего',
	noRefund: 'Возврат недоступен',
	feeCoversAmount: 'Комиссия покрывает всю сумму, возврата нет',
	refund: 'Возврат',
	viewEvent: 'Смотреть событие',
	cancel: 'Отмена',
	cancelBooking: 'Отменить бронь',
	areYouSure: 'Вы уверены?',
	youWillReceive: 'Вы получите',
	yesCancel: 'Да, отменить',
	close: 'Закрыть',
	confirmBooking: 'Подтвердить бронь',
	selectDate: 'Выберите дату',
	selectedDate: 'Выбранная дата',
	soldOut: 'Распродано',
	alreadyBooked: 'Уже забронировано',
	spotsLeft: 'мест осталось',
	selectSpots: 'Выберите места',
	totalPrice: 'Итоговая цена',
	spotsAvailable: 'мест доступно',
	preparingPayment: 'Готовим оплату...',
	continueToPayment: 'Перейти к оплате',
	paymentStartFailed: 'Не удалось начать оплату. Попробуйте снова.',
	createFailed: 'Не удалось создать бронь. Попробуйте снова.',
	completePayment: 'Завершить оплату',
	cardDetails: 'Введите данные карты, чтобы подтвердить бронирование.',
	amount: 'Сумма',
	quantity: 'Количество',
	phone: 'Номер телефона',
	confirming: 'Подтверждаем...',
	payNow: 'Оплатить',
	confirmPaymentTitle: 'Подтвердите оплату',
	confirmPaymentMessage: 'Вы уверены, что хотите оплатить {amount}?',
	confirmPaymentNote: 'Сумма будет списана с вашей карты сразу после подтверждения.',
	confirmPaymentBack: 'Назад',
	confirmPaymentPay: 'Оплатить {amount}',
	invalidPhone: 'Введите корректный номер телефона',
	filterAll: 'Все',
	filterConfirmed: 'Подтвержденные',
	filterCancelled: 'Отменённые',
	noBookingsMatchFilter: 'По этому фильтру бронирований нет.',
	paymentFailed: 'Платеж не прошел. Попробуйте снова.',
	paymentSubmitted: 'Платеж отправлен. Ждем подтверждения...',
	paymentInProgress: 'Оплата уже выполняется. Пожалуйста, дождитесь завершения.',
	paymentConfirmed: 'Платеж подтвержден. Бронирование готово.',
	paymentCancelled: 'Бронь не удалось завершить. Если списание было, сумма вернется автоматически.',
	paymentTimeout: 'Платеж занимает больше времени, чем ожидалось. Проверьте брони позже.',
	bookingExpired: 'Бронь истекла из-за неактивности.',
	stripeMissing: 'Stripe настроен некорректно. Свяжитесь с поддержкой.',
	unknownEvent: 'Неизвестное событие',
	unknownCustomer: 'Неизвестный клиент',
	unknownUser: 'Неизвестный пользователь',
	noEmail: 'Нет email',
	noLocation: 'Нет места',
	successTitle: 'Поздравляем!',
	successCongrats: 'Вы успешно забронировали «{event}». Место закреплено за вами.',
	successEmailSent: 'Подтверждение отправлено на {email}',
	paid: 'Оплачено',
	paymentMethod: 'Способ оплаты',
	paymentMethodSite: 'Оплата на сайте',
	paymentMethodOffline: 'Оплачено офлайн',
	paymentMethodOnArrival: 'Оплата на месте',
	viewMyBookings: 'Мои бронирования',
	refCopied: 'Код брони скопирован',
	updateFailed: 'Не удалось обновить бронь',
	cancelFailed: 'Не удалось отменить бронь',
	updateSuccess: 'Бронь обновлена',
	cancelSuccess: 'Бронь отменена',
};
